...
- **Infrastructure**: Repositorios, almacenamiento JSON, configuración de persistencia
...
- .NET 8.0 SDK
- Node.js 18+ y npm
- **No se requiere SQL Server** - Los datos se almacenan en archivos JSON
...
3. **Infrastructure**: Implementa los repositorios usando almacenamiento JSON y otras tecnologías de infraestructura. Depende de Domain y Application.
...
Los datos se almacenan en archivos JSON en la carpeta `Data/` del proyecto API:
- `Users.json` - Usuarios registrados
- `Categories.json` - Categorías de gastos
- `PaymentMethods.json` - Métodos de pago
- `Expenses.json` - Gastos registrados
- `Budgets.json` - Presupuestos configurados

Los archivos se crean automáticamente cuando se ejecuta la aplicación por primera vez.
